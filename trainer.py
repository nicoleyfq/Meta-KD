...
class Trainer:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    # kd change -----------
                    tr_loss_step = self.training_step(model, inputs, kd_model)
    ...
    def _prepare_inputs(self, inputs: Dict[str, Union[torch.Tensor, Any]]) -> Dict[str, Union[torch.Tensor, Any]]:
        ...
        return inputs
    # kd change -------------------
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], kd_model:None) -> torch.Tensor:
    # --------------------------------------
    # def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.use_amp:
            ...
            inputs['labels'] = None
            kd_output = kd_model(**inputs)
            
            kd_loss = kd_loss_func(kd_output['logits'], output[1])
            if do_kd:
                loss += kd_coef * kd_loss
            # ----------------------------------------
        if self.args.n_gpu > 1:
